...
void CoordinateTransition::LatLonToUTMXY(const double lat, const double lon, double &utm_x, double &utm_y) const
{
...
void CoordinateTransition::UTMXYToLatLon(double utm_x, double utm_y,double &lat, double &lon) const
{
...
void CoordinateTransition::ThetaToHeading(const double theta_radian, double velocity_x, double velocity_y, double &heading) const
{
  // Stationary state
...
void CoordinateTransition::MathLanLonToXY(const double lat_radian, const double lon_radian, const double central_meridian_radian, double &x, double &y) const
{
  // Precalculate ep2
...
void CoordinateTransition::MathXYToLanLon(const double x, const double y, const double central_meridian_radian, double &lat_radian, double &lon_radian) const
{
...
double CoordinateTransition::FootpointLatitude(const double y) const
{
  double result = 0.0;
...
double CoordinateTransition::ArcLengthOfMeridian(const double lat_radian) const
{
  double result = 0.0;
  // Precalculate n
  double n = (kEmA - kEmB) / (kEmA + kEmB);
...
double CoordinateTransition::UTMCentralMeridian(const double lon) const
{
  double zone = 0.0;
  if (lon < 0.0)
  {
