...
class CoordinateTransition {
 public:
  ...
  void UTMXYToLatLon(double utm_x, double utm_y, double& lat, double& lon) const;
  ...
  void ThetaToHeading(const double theta_radian, double velocity_x,
                      double velocity_y, double& heading) const;
 ...
 private:
  ...
  double FootpointLatitude(const double y) const;
  // Function that computes the ellipsoidal distance from the equator to
  // a point at a given latitude
  double ArcLengthOfMeridian(const double lat_radian) const;
  // Function that computes CentralMeridian
  double UTMCentralMeridian(const double lon) const;
  int zone_;
  bool southhemi_;
};
